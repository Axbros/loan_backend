...
graph:
	...
	@cp -f cmd/loan/main.go .
	go-callvis -skipbrowser -format=svg -nostd -file=loan loan
	@rm -f main.go loan.gv
...
.PHONY: build
# Build loan for linux amd64 binary
build:
	@echo "building 'loan', linux binary file will output to 'cmd/loan'"
	@cd cmd/loan && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build
