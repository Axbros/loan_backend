...
type LoanDisbursements struct {
	...
	BaseinfoID           int        `gorm:"column:baseinfo_id;type:int(11);not null" json:"baseinfoID"`                 // 关联申请单 loan_baseinfo.id
	DisburseAmount       int        `gorm:"column:disburse_amount;type:int(11);not null" json:"disburseAmount"`         // 放款金额(单位按你的系统：元/分，建议统一)
	NetAmount            int        `gorm:"column:net_amount;type:int(11);not null" json:"netAmount"`                   // 到账金额(扣除费用后实际到账)
	...
	AuditorUserID        int64      `gorm:"column:auditor_user_id;type:bigint(20)" json:"auditorUserID"`                // 审核人员(loan_users.id)
	AuditedAt            *time.Time `gorm:"column:audited_at;type:datetime" json:"auditedAt"`                           // 审核通过时间
	PayoutChannelID      int64      `gorm:"column:payout_channel_id;type:bigint(20)" json:"payoutChannelID"`            // 放款渠道(代付) loan_payment_channels.id
