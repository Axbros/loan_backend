...
type LoanRepaymentSchedules struct {
	...
	PrincipalDue   int        `gorm:"column:principal_due;type:int(11);default:0;not null" json:"principalDue"`   // 应还本金(建议统一单位：分)
	InterestDue    int        `gorm:"column:interest_due;type:int(11);default:0;not null" json:"interestDue"`     // 应还利息(分)
	FeeDue         int        `gorm:"column:fee_due;type:int(11);default:0;not null" json:"feeDue"`               // 应还费用(分)
	PenaltyDue     int        `gorm:"column:penalty_due;type:int(11);default:0;not null" json:"penaltyDue"`       // 应还罚息(分，逾期产生)
	TotalDue       int        `gorm:"column:total_due;type:int(11);not null" json:"totalDue"`                     // 本期应还总额=本金+利息+费用+罚息(分)
