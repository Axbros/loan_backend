...
type LoanPaymentChannels struct {
	...
	PayoutFeeRate    int    `gorm:"column:payout_fee_rate;type:int(11)" json:"payoutFeeRate"`                // 代付手续费率(如35=35%)
	PayoutFeeFixed   int    `gorm:"column:payout_fee_fixed;type:int(11)" json:"payoutFeeFixed"`              // 代付固定手续费(分，若不用可为空)
	CollectFeeRate   int    `gorm:"column:collect_fee_rate;type:int(11)" json:"collectFeeRate"`              // 代收手续费率
