...
func newLoanSettingsHandler() *gotest.Handler {
	...
	d.IDao = dao.NewLoanSettingsDao(d.DB, c.ICache.(cache.LoanSettingsCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &loanSettingsHandler{iDao: d.IDao.(dao.LoanSettingsDao)}
	iHandler := h.IHandler.(LoanSettingsHandler)
...
func Test_loanSettingsHandler_Create(t *testing.T) {
	h := newLoanSettingsHandler()
	defer h.Close()
	testData := &types.CreateLoanSettingsRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.LoanSettings))
...
func Test_loanSettingsHandler_DeleteByID(t *testing.T) {
	h := newLoanSettingsHandler()
	defer h.Close()
	testData := h.TestData.(*model.LoanSettings)
...
func Test_loanSettingsHandler_UpdateByID(t *testing.T) {
	h := newLoanSettingsHandler()
	defer h.Close()
	testData := &types.UpdateLoanSettingsByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.LoanSettings))
...
func Test_loanSettingsHandler_GetByID(t *testing.T) {
	h := newLoanSettingsHandler()
	defer h.Close()
	testData := h.TestData.(*model.LoanSettings)
...
func Test_loanSettingsHandler_List(t *testing.T) {
	h := newLoanSettingsHandler()
	defer h.Close()
	testData := h.TestData.(*model.LoanSettings)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListLoanSettingssRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewLoanSettingsHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewLoanSettingsHandler()
}
